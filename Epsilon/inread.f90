...
!#BEGIN_INTERNAL_ONLY
    ...
    elseif(trim(keyword).eq.'os_hdf5') then
      pol%os_hdf5 = .true.
!#END_INTERNAL_ONLY
...
! for the moment subspace works only in combination with MPI and SCALAPACK
  ...
  IF(pol%subspace) THEN
    if (peinf%inode==0) then
      write(0,*)
      write(0,*) 'WARNING: Satatic Subspace method only works with MPI and SCALAPACK.'
...
!#BEGIN_INTERNAL_ONLY
  ...
  on_steroids=pol%os_opt_ffts/=0.or.pol%os_hdf5
  ...
  if (peinf%inode==0) then
    ...
    write(6,'(1x,a,l1)') ' - Use HDF5 Parallel IO: ', pol%os_hdf5
#ifndef HDF5
    if(pol%os_hdf5)  call die("Must build code with HDF5 to use os_hdf5 option.", only_root_writes = .true.)
#endif    
    if (on_steroids) then
...
! JRD XXX We should write out frequency grid somewhere
      ...
      if(pol%do_rpa) then
        ...
        SAFE_ALLOCATE(pol%rpa_freq_grid, (pol%nfreq_imag)) 
        SAFE_ALLOCATE(omega_CC, (pol%nfreq_imag))
...
805 format(1x,'We are not communicating',/)
  ...
  if (peinf%inode==0 .and. pol%intraband_flag/=0) then
    if (pol%intraband_flag<0 .or. pol%intraband_flag>4) &
